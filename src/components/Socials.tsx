...
export default function Socials({ className }: { className?: string; invert?: boolean }) {
  return (
    <FadeInStagger role="list" className={clsx('flex gap-x-4 text-white mt-5', className)}>
      {myself.socials.map((socialMediaProfile) => (
        <FadeIn key={socialMediaProfile.name}>
          <Link href={socialMediaProfile.url} target="_blank" aria-label={socialMediaProfile.title} className="transition hover:text-white/80 duration-200">
