...
import { BookHeart, Flame, Images, LaptopMinimal, UserPen } from 'lucide-react';
...
const subSectionsIcons: { [key: string]: JSX.Element } = {
  ...
  skills: <Technologies />,
  'my-work': <Projects />,
  'achievements': <Ellipsis />,
  'profile': <UserPen className='h-4 text-white' />,
  'loadout': <LaptopMinimal  className='h-4 text-orange-400' />,
  'gallery': <Images className='h-4 text-blue-100' />,
      ...
      {expanded && (
        <>
          ...
          <Folder name="src" openIcon={<SrcOpen />} closedIcon={<Src />} indent={0} segmentActive={false}>
            <File name="personal-info.tsx" icon={<BookHeart className='w-4 h-4 text-pink-500' /> } url="/personal-info" indent={1} sections={pathname === '/personal-info' ? sections : []} />
  ...
  return (
    <AnimatePresence mode="popLayout" initial={true}>
      ...
      >
        ...
        <Link href={`${url}#${id}`} className={clsx('flex items-center hover:text-white px-[4px] p-0.5 transition-colors duration-300 hover:bg-dark_border/50 rounded-sm mb-0.5', isVisible ? 'text-blue-100' : 'text-gray-500')}>
